    ...
    <v-card>
      <v-card-title>
        <v-input>
          ...
          <v-btn depressed height="40" color="teal darken-3" class="rounded-l-0" @click="getProducts()">
      ...
      <v-card-text class="font-weight-light">
        <v-data-table :headers="headers" :items="products" hide-default-footer>
          <template v-slot:item.ver_historial={item} depressed>
            <v-btn color="teal darken-3" @click="getHistorial(item)" depressed>Ver historial</v-btn>
          </template>
        </v-data-table>
      </v-card-text>
    </v-card>
