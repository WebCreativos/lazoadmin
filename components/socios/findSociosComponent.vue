<template>
  <v-combobox full-width class="font-weight-bold" solo no-filter :hide-details="hideDetails" :items="socios" dense
    ...
    <template v-slot:item="data">
      <b>Nombre:</b> {{data.item.name}}<span v-show="data.item.address"> | <b>Direccion:</b>
  ...
  export default {
    ...
    watch: {
      search(val) {
        ...
        const query =
          `?_where[_or][0][name_contains]=${val}&_where[_or][1][last_name_contains]=${val}&_where[_or][3][user.username_contains]=${val}`
        this.$axios
          .get(`/socios/${query}`)
          .then(data => {
