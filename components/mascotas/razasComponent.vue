  ...
  <div>
    <v-input hide-details>
      <v-autocomplete small-chips hide-details dense :items="razasList" :class="(!hideAddMore)?'rounded-r-0':''" label="Raza" item-text="nombre" item-value="nombre"  outlined
        ...
        <v-btn class="rounded-l-0 rounded-r-lg" v-if="!hideAddMore" depressed height="40" color="teal darken-3" @click="showrazasModal = true">
    ...
    <v-dialog v-model="showrazasModal">
      <v-card width="800">
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn class="white--text" color="teal darken-3 font-weight-light rounded-lg" @click="addRaza()">
            AGREGAR
          </v-btn>
        </v-card-actions>
      </v-card>
